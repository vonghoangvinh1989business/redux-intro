...
const RootComponent = (props) => {
  ...
  const addProductToCard = (productItem) => {
    const newProductList = cart.products.map((cardProduct) => {
      if (cardProduct.id === productItem.id) {
        cardProduct.price += productItem.price;
        cardProduct.qty += 1;
      }
      return cardProduct;
    });

    const newTotalPrice = cart.totalPrice + productItem.price;
  ...
  const removeProductFromCart = (productItem) => {
    let newTotalPrice = cart.totalPrice;
    const newProductList = cart.products.map((cardProduct) => {
      if (cardProduct.id === productItem.id && cardProduct.qty > 0) {
        cardProduct.price -= productItem.price;
        cardProduct.qty -= 1;
        newTotalPrice -= productItem.price;
      }
      return cardProduct;
    });
    setCart({ products: newProductList, totalPrice: newTotalPrice });
